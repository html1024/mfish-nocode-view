...
import { useDesign as U, useRootSetting as j } from "@mfish/core/hooks";
import { u as w, _ as A, f as B, D as L } from "./index.js";
...
const G = ["title", "onClick", "onMouseenter"], N = { class: "title" }, F = { class: "title" }, q = { class: "operator" }, H = { key: 1, class: "param-edit" }, J = { key: 2, class: "error" }, K = A(e({ __name: "GlobalParamsSelect", props: { value: { type: Object, default: () => ({}) } }, emits: ["selectChange"], setup(e2, { emit: _2 }) {
  a((e3) => ({ v75000e46: j2.value }));
  const S2 = e2, z2 = _2, { prefixCls: M2 } = U("global-params-select"), P2 = w(), T2 = l([]), { token: V2 } = x.useToken(), j2 = n(() => V2.value.colorInfoBg), A2 = l(-1), B2 = l(-1), L2 = l(-1), O2 = l(), K2 = t("scrollContainerRef"), Q2 = l();
  function W() {
    X() && (T2.value.push({ name: "", defaultValue: "" }), B2.value = T2.value.length - 1, A2.value = B2.value, K2.value?.scrollBottom());
  }
  function X() {
    if (-1 === B2.value) return O2.value = "", true;
    const e3 = T2.value[B2.value].name;
    ...
    return T2.value.some((a2, l2) => l2 !== B2.value && a2.name === e3) ? (O2.value = "error", Q2.value = "参数名不能重复", false) : (P2.getCanvasConfig.globalParams = R(T2.value), z2("selectChange", R(T2.value[B2.value])), B2.value = -1, O2.value = "", Q2.value = "", true);
  }
  function Y(e3) {
    T2.value.splice(e3, 1), P2.getCanvasConfig.globalParams = R(T2.value), B2.value = -1;
  }
  return o(() => P2.getCurConfigComponent, (e3) => {
    e3 && (T2.value = R(P2.getCanvasConfig.globalParams || []), A2.value = T2.value.findIndex((e4) => e4.name === S2.value.name));
  }, { immediate: true }), (e3, a2) => {
    const l2 = u("AButton");
    return i(), s("div", { class: r(v(M2)) }, [T2.value.length > 0 ? (i(), c(v(D), { key: 0, ref_key: "scrollContainerRef", ref: K2, class: r(`${v(M2)}-items`) }, { default: d(() => [(i(true), s(p, null, m(T2.value, (e4, n2) => (i(), s(p, { key: n2 }, [B2.value !== n2 ? (i(), s("div", { key: 0, class: r(["item", A2.value === n2 ? "select" : ""]), title: e4.name, onClick: (e5) => (function(e6) {
      A2.value = e6, z2("selectChange", R(T2.value[e6]));
